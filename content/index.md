# Hi, I'm Jason Hong

I'm a course 6 (Computer Science) student at MIT. I'm deeply interested
in performance engineering and building reliable systems that can
maximize our machines.


```python
"""
The design of this page is a special ode to MIT's CATSOOP, 
the online learning platform 
that Course 6 students know all too well.
"""
```

<div style="display:flex;gap:1rem;margin-top:1.5rem;">
<a href="/projects/" class="btn">View Projects</a>
<a href="/blog/" class="btn">Read Blog</a>
<a href="/resume/" class="btn">Resume</a>
</div>


## Recent Posts

- [Drinking from the firehose](/blog/classes-spring-2026)

## My Setup

- **Editor:** Neovim with [kickstart.nvim](https://github.com/nvim-lua/kickstart.nvim)
- **Keyboard:** Kinesis Advantage360
- **Mouse:** Logitech G Pro X Superlight
- **Laptop:** MacBook Pro with M3 chip

## Featured Projects

- Coming soon!
